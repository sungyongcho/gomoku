...
The current champion model was trained from scratch in the fourth of four training runs on GCP; runs 1-3 did not produce it, but each exposed a different failure mode that shaped the next attempt's hyperparameters. The configs were updated mid-run via the scheduled parameter system — sometimes daily — based on evaluation results from the arena.

Training was run on the Ray cluster profile defined in [`alphazero/infra/cluster/cluster_elo1800.yaml`](https://github.com/sungyongcho/gomoku/blob/main/alphazero/infra/cluster/cluster_elo1800.yaml): one `g2-standard-16` head node (`CPU:16`, one NVIDIA L4 GPU) plus 8-vCPU CPU workers: eight `c4-highcpu-8` until 2026-02-04, then ten `c4-standard-8` as in this profile.
...
The day 6 rule bug is worth highlighting: the model trained for 135 iterations under subtly wrong rules, and the resulting play was still visually plausible. Without the replay data analysis that measured the contamination rate (a White double-three point in about 1.8% of sampled White-turn positions overall, and 7-11% in the latest days), the bug might never have been caught. This reinforced the unit-testing lesson from the refactoring — gameplay testing is not sufficient.
...
- **200 MCTS simulations per move** — strong CPU play on the `c2d-standard-4` deployment VM
