...
PVS was implemented with help from the [chessprogramming.org PVS article](https://www.chessprogramming.org/Principal_Variation_Search) for the null-window re-search logic. The reference pseudocode uses negamax with `(-alpha-1, -alpha)`; this engine's non-negamax minimax form passes `(alpha+1, alpha+1)` instead: a width-0 window rather than the reference's width-1 null window, for the reasons explained above.
