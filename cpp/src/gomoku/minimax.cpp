...
namespace Minimax
{
	...
	int evaluateCombinedPattern(int combinedPattern, int player)
	{
		...
		// Check open ends: if the cell immediately outside the contiguous run is EMPTY.
		bool openLeft = (center - leftCount - 1 >= 0 && cells[center - leftCount - 1] == EMPTY_SPACE);
		bool openRight = (center + rightCount + 1 < COMBINED_WINDOW_SIZE && cells[center + rightCount + 1] == EMPTY_SPACE);
		...
		// Forward capture check: if cells at indices center+1, center+2, center+3 equal: opponent, opponent, player.
		if (center + 3 < COMBINED_WINDOW_SIZE)
		{
			if (cells[center + 1] == opponent &&
				cells[center + 2] == opponent &&
				cells[center + 3] == player)
			{
				score += CAPTURE_SCORE;
			}
		}
		// Backward capture check: if cells at indices center-3, center-2, center-1 equal: player, opponent, opponent.
		if (center - 3 >= 0)
		{
			if (cells[center - 1] == opponent &&
				cells[center - 2] == opponent &&
	...
	int evaluateCombinedAxis(Board *board, int player, int x, int y, int dx, int dy)
	{
		...
		// Reverse the backward window so that the cell immediately adjacent to (x,y) is at the rightmost position.
		unsigned int revBackward = reversePattern(backward, SIDE_WINDOW_SIZE);
		// Combine: [reversed backward window] + [center cell (player)] + [forward window]
		unsigned int combined = (revBackward << (2 * (SIDE_WINDOW_SIZE + 1))) |
								((unsigned int)player << (2 * SIDE_WINDOW_SIZE)) |
								forward;
	...
	int evaluatePosition(Board *&board, int player, int x, int y)
	{
		...
		if (board->getValueBit(x, y) == EMPTY_SPACE)
			return 0;
