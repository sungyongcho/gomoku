...
if CHECKPOINT_PATH.is_file():  # ▸ 체크포인트 있으면 로드
    ...
    checkpoint = torch.load(
        CHECKPOINT_PATH,
        map_location=lambda storage, loc: torch.device("cpu"),
        weights_only=False
    )
...
_model.eval()
_mcts = PVMCTS(_model, sims=160, device="cpu")
...
if CHECKPOINT_PATH.is_file():  # ▸ 체크포인트 있으면 로드
    _model.load_state_dict(
        torch.load(CHECKPOINT_PATH, map_location="cpu", weights_only=False)["model_state_dict"]
    )
    print(f"[AI] checkpoint loaded → {CHECKPOINT_PATH}")
else:  # ▸ 없으면 랜덤 초기값
    print("[AI] checkpoint not found – using fresh weights")
