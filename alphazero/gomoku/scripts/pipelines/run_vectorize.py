...
def run_vectorize(
    ...
    *,
    ...
    """Vectorized self-play using batch PVMCTS."""
    ...
    runner = VectorizeRunner(game=game, mcts_cfg=params.mcts, train_cfg=params.training)
    ...
    if loaded_shards:
        print(
            f"[Vectorize] Warm-started replay buffer with {len(loaded_shards)} shard(s)."
        )
    ...
    for it in range(num_iters):
        ...
        if sched["num_games"] <= 0:
            print("[Vectorize] No self-play games scheduled; skipping iteration.")
            if manifest_updater:
                manifest_updater(
                    params,
                    fs,
        ...
        batch_size = params.parallel.num_parallel_games or params.mcts.batch_infer_size
        ...
        if prev_bot_games > 0:
            ...
            if past_ckpt:
                try:
                    ...
                    print(f"[Vectorize] Loaded past opponent from {past_ckpt}")
                except Exception as exc:  # noqa: BLE001
                    print(
                        f"[Vectorize] Failed to load past opponent at {past_ckpt}: {exc}"
                    )
                    past_agent = None
            else:
                print("[Vectorize] No past checkpoint found; falling back to self-play.")

        def _run_games(count: int, opponent=None, agent_first=True):  # noqa: ANN001
            acc: list = []
            remaining = count
            local_agent_first = agent_first
            while remaining > 0:
                games_now = min(batch_size, remaining)
                acc.extend(
                    runner.play_batch_games(
                        ...
                        temperature=sched["temp"],
                        add_noise=True,
                        target_games=games_now,
                        progress_desc="Self-Playing vs Champion",
                        random_ratio=sched["rnd_ratio"],
                        random_opening_turns=sched["rnd_opening"],
                        opponent=opponent,
                        agent_first=local_agent_first,
                    )
                )
                remaining -= games_now
                local_agent_first = not local_agent_first
            return acc, local_agent_first

        records: list = []
        agent_first_flag = True
        random_records, agent_first_flag = _run_games(
            rnd_bot_games, opponent=random_bot, agent_first=agent_first_flag
        )
        records.extend(random_records)
        self_records, agent_first_flag = _run_games(
            prev_bot_games,
            opponent=past_agent,
            agent_first=agent_first_flag,
        )
        ...
        if not replay_buffer:
            print("[Vectorize] Replay buffer empty; skipping training.")
            continue
