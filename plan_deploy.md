...
Cloudflare (TLS termination + path routing via Worker)
  ...
  │  /minimax/*  ──► GCP e2-micro VM (minimax, port 8080)
...
**Note (important)**: Cloudflare only proxies WebSockets to a limited set of origin ports. `8005` is not reliably supported, so for deployment we should run minimax on `8080` on its VM. Local development can stay on `8005`.
...
- Stage 2 (runtime): debian:bookworm-slim + libwebsockets runtime package (name varies by Debian version, e.g. `libwebsockets17` on bookworm) → `CMD ["./minimax"]`
- `ENV MINIMAX_PORT=8080`, `EXPOSE 8080` (keeps Cloudflare-compatible port; can still be overridden at runtime)

### 2. `alphazero/infra/image/Dockerfile.prod` — Production alphazero serving image
...
### 4. `deploy/env_config.sh` — Load and validate `.env`
...
### 5. `deploy/setup.sh` — One-time GCP provisioning
...
4. Create firewall rule (`gomoku-deploy` tag) allowing TCP 8080 from Cloudflare IP ranges + SSH from IAP
...
### 6. `deploy/deploy.sh` — Build, push, and update containers
...
3. Copy model checkpoint into build context:
   - Ensure destination is writable (it may be root-owned from previous docker runs): `rm -f alphazero/models/champion.pt`
   - Copy file: `cp -L alphazero/runs/elo1800-gcp-v4/ckpt/champion.pt alphazero/models/champion.pt`
   - Do not commit this file (recommend: stop tracking it and add it to `.gitignore`, keeping `alphazero/models/.gitkeep`). One-time: `git rm --cached alphazero/models/champion.pt` + add `alphazero/models/champion.pt` to `.gitignore`.
4. Build minimax image: `docker buildx build -f minimax/Dockerfile.prod minimax/`
5. Build alphazero image: `docker buildx build -f alphazero/infra/image/Dockerfile.prod alphazero/`
...
### 7. `deploy/teardown.sh` — Remove all deploy resources
...
### 8. `deploy/cloudflare-worker.js` — Path-based routing Worker
...
- `/minimax/*` → strip prefix, forward to `http://MINIMAX_IP:8080/*`
- `/gomoku/*` → pass through (reserved for frontend)
- `/*` → pass through to GitHub Pages origin

**No changes to `sungyongcho.github.io` repo needed.** The Worker intercepts matching paths at the Cloudflare edge *before* requests reach any origin. GitHub Pages continues serving `sungyongcho.com/` as usual — it never sees `/minimax` or `/alphazero` requests. The Worker + `wrangler.toml` live in `gomoku/deploy/` and are deployed via `npx wrangler deploy`.

### 9. `deploy/wrangler.toml` — Cloudflare Worker config
...
```python
NUM_SEARCHES: int | None = int(os.getenv("ALPHAZERO_MCTS_NUM_SEARCHS", "200"))
```

### 3. `alias.sh` — Add deployment aliases
```bash
## deployment
alias deploy-setup="bash deploy/setup.sh"
alias deploy="bash deploy/deploy.sh"
alias deploy-teardown="bash deploy/teardown.sh"
...
Document the new `deploy/` structure, env vars, and workflow.
...
cd deploy && npx wrangler deploy        # Deploy the Worker
...
1. **Local Docker test**: Build both prod images, run locally, test with `wscat -c ws://localhost:8005/ws` (minimax, set `MINIMAX_PORT=8005`) and `ws://localhost:8080/ws` (alphazero)
2. **Direct VM test**: After `deploy`, test `wscat -c ws://<MINIMAX_VM_IP>:8080/ws` and `ws://<ALPHAZERO_VM_IP>:8080/ws`
