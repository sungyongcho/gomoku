...
class SequentialEngine(SearchEngine):
        ...
        **kwargs,
        ...
        if self.use_native:
            ...
            try:
                ...
                raise RuntimeError(
                    "gomoku_cpp extension is not available; rebuild the package."
                ) from exc

            self._native_engine = gomoku_cpp.MctsEngine(
                game._native_core, float(mcts_params.C)
            )
    ...
    def _infer_single(
        ...
        state_tensor = self._encode_state_to_tensor(
            node_state, dtype=dtype, device=device
        )
        ...
        policy = self._masked_policy_from_logits(
            logits, legal_mask, apply_dirichlet=apply_dirichlet
        )
        return policy, value
